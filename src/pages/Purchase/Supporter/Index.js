...
class Supporter extends Component {
  ...
  fetchList = (params = {}) => {
    const { dispatch } = this.props;
    dispatch({ type: 'supporter/fetch', payload: params });
  };
  ...
  editHandler = (id, values) => {
    const { dispatch } = this.props;
    dispatch({ type: 'supporter/update', payload: { id, ...values } });
  };

  // 生成合同
  contractHandler = values => {
    const { dispatch } = this.props;
    dispatch({ type: 'supporter/createContractorders', payload: { ...values } });
  };
  ...
  examineHandler = (id, fields) => {
    const { dispatch } = this.props;
    dispatch({ type: 'supporter/update', payload: { id, ...fields } });
  };
  ...
  handleDelete = fields => {
    const { dispatch } = this.props;
    dispatch({ type: 'supporter/update', payload: { id: fields.id, status: 2 } });
  };

  render() {
    const {
      supporter: { list, pagination },
      dispatch,
    } = this.props;
